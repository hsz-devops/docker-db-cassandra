#!/usr/bin/env bash
#
# v1.0.0-cassandra    2017-11-23     webmaster@highskillz.com
#
set -e
set -o pipefail
#set -x

SFD_ENTRYPOINT="${SAFEDIR__ENTRYPOINT:-/docker-entrypoint.sh}"

SAFEDIR__DIRPATH="${SAFEDIR__DIRPATH:-/var/lib/cassandra}"
...
if [ "${SAFEDIR__DISABLE_CHECKS}" != "1" ]; then
    ...
    #if check_folder_is_safe "${SAFEDIR__DIRPATH}/commitlog"    "NOTAG" ; then echo OK; else exit "Not safe [2]"; exit -52; fi
    #if check_folder_is_safe "${SAFEDIR__DIRPATH}/data"         "NOTAG" ; then echo OK; else exit "Not safe [3]"; exit -53; fi
    #if check_folder_is_safe "${SAFEDIR__DIRPATH}/saved_caches" "NOTAG" ; then echo OK; else exit "Not safe [4]"; exit -54; fi
...
exec_via__init() {
    ...
    set -x
    # reusing cassandra's entrypoint
    exec /usr/local/bin/dumb-init -- /docker-entrypoint.sh "$@"
}
exec_via__init "${SFD_ENTRYPOINT}" "$@"
